    ...
    if (calendar_selector.length > 0) {
        ...
        var calendar_settings = {
            ...
            dayClick: function (date, jsEvent, view) {
                var d = date.format();
                var persiandate = moment(d).format('jYYYY/jMM/jDD HH:mm');
                ...
                $("input[name='start'].datetimepicker").val(persiandate);
        ...
        if (message != '' && number != '') {
            ...
            $.post(window.location.href, {
                ...
                if (response.success == true) {
                    errorContainer.html('<div class="alert alert-success no-mbot mtop15">پیام شما ارسال شد</div>');
                } else {
...
function edit_reminder(id, e) {
    requestGetJSON('misc/get_reminder/' + id).done(function (response) {
        ...
        var persiandate = moment(response.date).format('jYYYY/jMM/jDD HH:mm');
        $container.find('form').attr('data-edit', true);
        $container.find('#description').val(response.description);
        $container.find('#date').val(persiandate);
