...
abstract class App_import
{
    ...
    public function downloadSampleFormHtml()
    {
        ...
        $form .= '<button type="submit" class="btn btn-success">دانلود نمونه</button>';
        $form .= '<hr />';
        $form .= form_close();
    ...
    {
        $this->addImportGuidelinesInfo('اگر ستون <b> ای که می خواهید وارد کنید تاریخ است مطمئن شوید که قالب آن قالب بندی شده است Y-m-d (' . date('Y-m-d') . ').</b>');

        $this->addImportGuidelinesInfo('داده های CSV شما باید در قالب زیر باشد. اولین خط فایل CSV شما باید عنوان ستون مانند مثال جدول باشد. همچنین مطمئن شوید که فایل شما هست <b>UTF-8</b> برای جلوگیری از موارد غیر ضروری <b>مشکلات کدگذاری</b>.');
    }
