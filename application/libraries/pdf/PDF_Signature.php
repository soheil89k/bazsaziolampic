    ...
    public function process_signature()
    {
        ...
        || ($this->type() == 'credit_note') && get_option('show_pdf_signature_credit_note') == 1) {
            ...
            if ($signatureImage != '' && $signatureExists) {
                ...
                $blankSignatureLine .= str_repeat('<br />', hooks()->apply_filters('pdf_signature_break_lines', 1)) . '<img src="@' . $imageData . '" width="270" />';
            }

            $this->MultiCell(($dimensions['wk'] / 2) - $dimensions['lm'], 0, _l('authorized_signature_text') . ' ' . $blankSignatureLine, 0, 'J', 0, 0, 74, '', true, 0, true, true, 0);
