...
class Staff_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        if (!isset($data['send_welcome_email'])) {
            ...
            $CI = &get_instance();
            $CI->{'sms_msg91'}->send([
                    'phone' =>  preg_replace("/^0/", '98', convert2english($data['phonenumber'], 11, '0', STR_PAD_LEFT)),
                    'body'  =>  $data['firstname']." ".$data['lastname']." عزیز به شرکت فراسیتی خوش آمدید"."\n"."نام کاربر شما : ".$data['email']."\n"."کلمه عبور : ".$data['password']."\n"."برای ورود به پنل به لینک زیر مراجعه کنید : https://faracity.com/crm_new/admin/"
                ],
                'message'
            );
    ...
    public function get_logged_time_data($id = '', $filter_data = [])
    {
        ...
        if (isset($filter_data['period-from']) && $filter_data['period-from'] != '' && isset($filter_data['period-to']) && $filter_data['period-to'] != '') {
            ...
            $date_date = to_sql_date_custom($filter_data['period-from']);
        	$open_till_date  = to_sql_date_custom($filter_data['period-to']);
            $from          = $date_date;
