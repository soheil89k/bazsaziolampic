...
class Estimates_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        $expirydate_date   = to_sql_date_custom($data['expirydate']);

        $data['date']  = $date_date;
        if ($data['expirydate'] == '') {
            unset($data['expirydate']);
        } else {
            $data['expirydate'] = $expirydate_date;
        }
        ...
        if (is_array($send_to) && count($send_to) > 0) {
            ...
            foreach ($send_to as $contact_id) {
                if ($contact_id != '') {
                    ...
                    $tmp = prepare_mail_preview_data($template_name, $estimate->clientid);


                    $currency = get_currency($estimate->currency);
                    $message = str_replace('{estimate_number}', format_estimate_number($estimate->id), $this->input->post('email_template_custom'));
                    ...
                    $message = str_replace('{estimate_subtotal}', app_format_money($estimate->subtotal, $currency), $message);
                    $message = str_replace('{estimate_date}', to_view_date_time_custom($estimate->date), $message);
                    $message = str_replace('{estimate_expirydate}', to_view_date_time_custom($estimate->expirydate), $message);
                    $message = str_replace('{estimate_link}', get_estimate_shortlink($estimate), $message);
                    ...
                    // $CI->{'sms_msg91'}->send([
                    //         'phone' =>  preg_replace("/^0/", '98', convert2english(str_pad($contact->phonenumber, 11, '0', STR_PAD_LEFT))),
                    //         'body'      => site_url('estimate/' . $estimate->id . '/' .  $estimate->hash),
                    //         'filename'  => 'پیش-فاکتور-فراسیتی.pdf',
                    //         'caption'   => 'پیش فاکتور',
                    //     ],
                    //     'sendFile'
                    // );
