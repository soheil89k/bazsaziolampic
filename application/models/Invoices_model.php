...
class Invoices_model extends App_Model
{
        ...
        for ($i = 1; $i <= 3; $i++) {
            ...
            if (isset($data['years']) && count($data['years']) > 0) {
                if(count($data['years']) > 1){
                    ...
                    $b = $y_arr[0];
                    $e = end($y_arr);
                    $begin = date('Y-m-d', $this->jdf->jmktime(0,0,0,1,1,$b));
                    $end = date('Y-m-d', $this->jdf->jmktime(0,0,0,12,29,$e));
                    var_dump($begin);
                    var_dump($end);
                }else{
                var_dump($data['years']);
                    $y_arr = $data['years'];
                    $s = $y_arr[0];
                    $begin = date('Y-m-d', $this->jdf->jmktime(0,0,0,1,1,$s));
                    $end = date('Y-m-d', $this->jdf->jmktime(0,0,0,12,29,$s));
                }
                $this->db->where('date  >=', $begin);
                $this->db->where('date <=', $end);
            } else {
                $begin = date('Y-m-d', $this->jdf->jmktime(0,0,0,1,1,$this->jdf->jdate('Y')));
                $end = date('Y-m-d', $this->jdf->jmktime(0,0,0,12,29,$this->jdf->jdate('Y')));
                // var_dump($end);die();
                $this->db->where('date  >=', $begin);
                $this->db->where('date <=', $end);
            }
    ...
    public function add($data, $expense = false)
    {
        ...
        $date_date = to_sql_date_custom($data['date']);
        $duedate_date   = to_sql_date_custom($data['duedate']);

        $data['date']  = $date_date;
        if ($data['duedate'] == '') {
            unset($data['duedate']);
        } else {
            $data['duedate'] = $duedate_date;
        }
        ...
        if (is_array($send_to) && count($send_to) > 0) {
            ...
            foreach ($send_to as $contact_id) {
                if ($contact_id != '') {
                    ...
                    $tmp = prepare_mail_preview_data($template_name, $invoice->clientid);

                    $currency = get_currency($estimate->currency);
                    $message = str_replace('{invoice_number}', format_estimate_number($invoice->id), $this->input->post('email_template_custom'));
                    $message = str_replace('{contact_firstname}', $contact->firstname, $message);
                    $message = str_replace('{contact_lastname}', $contact->lastname, $message);
                    $message = str_replace('{invoice_subtotal}', app_format_money($invoice->subtotal, $currency), $message);
                    $message = str_replace('{invoice_date}', to_view_date_time_custom($invoice->date), $message);
                    $message = str_replace('{invoice_expirydate}', to_view_date_time_custom($invoice->expirydate), $message);
                    $message = str_replace('{invoice_link}', get_invoice_shortlink($invoice), $message);
                    ...
                    // $CI->{'sms_msg91'}->send([
                    //         'phone' =>  preg_replace("/^0/", '98', convert2english(str_pad($contact->phonenumber, 11, '0', STR_PAD_LEFT))),
                    //         'body'      => site_url('invoice/' . $invoice->id . '/' .  $invoice->hash),
                    //         'filename'  => 'فاکتور-فراسیتی.pdf',
                    //         'caption'   => 'فاکتور',
                    //     ],
                    //     'sendFile'
                    // );
    ...
    * Get the invoices years
     ...
     * @return array
     */
    public function get_p_invoices_years()
    {
        $p_years = $this->db->query('SELECT DISTINCT((date)) as year FROM ' . db_prefix() . 'invoices ORDER BY year DESC')->result_array();
        return $p_years;
    }
