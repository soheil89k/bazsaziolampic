...
class Tasks_model extends App_Model
{
    ...
    public function add($data, $clientRequest = false)
    {
        ...
        $data['startdate'] = to_sql_date_custom($data['startdate']);
        $data['duedate']   = to_sql_date_custom($data['duedate']);
    ...
    public function timesheet($data)
    {
        if (isset($data['timesheet_duration']) && $data['timesheet_duration'] != '') {
            ...
            $start_date = to_sql_date_custom($data['start_time'], true);
			$end_date   = to_sql_date_custom($data['end_time'], true);

            $start_time = to_sql_date($start_date, true);
            $end_time   = to_sql_date($end_date, true);
