...
class Contracts_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        $datestart_date = to_sql_date_custom($data['datestart']);
        $dateend_date   = to_sql_date_custom($data['dateend']);
        
        $data['datestart']  = $datestart_date;
        if ($data['dateend'] == '') {
            unset($data['dateend']);
        } else {
            $data['dateend'] = $dateend_date;
        }
    ...
    */
            ...
            foreach ($sent_to as $contact_id) {
                if ($contact_id != '') {
                    ...
                    $CI->{'sms_msg91'}->send([
                            ...
                            'body'  =>  $this->input->post('email_template_custom')."\n".'لینک : '.site_url('contract/' . $contract->id . '/' .  $contract->hash)
                        ],
                        'message'
                    );
                    ...
                    $CI->{'sms_twilio'}->send(
                        ...
                        $this->input->post('email_template_custom')."\n".site_url('contract/' . $contract->id . '/' .  $contract->hash)
                    );
    ...
    public function renew($data)
    {
        ...
        $datestart_date = to_sql_date_custom($data['new_start_date']);
		$dateend_date   = to_sql_date_custom($data['new_end_date']);

        $data['new_start_date']  = $datestart_date;
        if (!is_date($data['new_end_date'])) {
            unset($data['new_end_date']);
        } else {
            $data['new_end_date'] = $dateend_date;
        }
