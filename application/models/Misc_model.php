...
class Misc_model extends App_Model
{
    ...
    public function add_reminder($data, $id)
    {
        ...
        $date_date = to_sql_date_custom($data['date'], true);
        $data['date']  = $date_date;
    ...
    public function add_note($data, $rel_type, $rel_id)
    {
        ...
        if($rel_type == "customer") {
            $contact = $this->clients_model->get_contacts($rel_id);
            $message = "یاداشت جدید توسط همکار ".get_staff_full_name(get_staff_user_id())." ایجادگرید"."\n"."نام مشتری : ".$contact[0]['firstname']." ".$contact[0]['lastname']."\n".strip_tags($data['description']);

            $CI = &get_instance();
            $CI->{'sms_msg91'}->send([
                    'phone' =>  "120363039592252689@g.us",
                    'body'  =>  $message
                ],
                'groupSend'
            );

            $customer = $this->db->where('customer_id =', $rel_id)->get(db_prefix() . 'customer_admins')->result_array();
