...
class Reports_model extends App_Model
{
    ...
    public function leads_monthly_report($month)
    {
        ...
        $data        = [];
        for ($d = 1; $d <= 31; $d++) {
            $time = $this->jdf->jmktime(12, 0, 0, $month, $d, $this->jdf->jdate('Y'));
            if ($this->jdf->jdate('m', $time,'','','en') == $month) {
				$gmonth = date('m', $time);
                $month_dates[] = $this->jdf->jdate('Y-m-d',convert_datetime(convert_date_utc_to_local(date('Y-m-d', $time))));
                $data[]        = 0;
            }
        }
        $result      = $this->db->query('select date_converted from ' . db_prefix() . 'leads where MONTH(date_converted) = ' . $gmonth . ' AND status = 1 and lost = 0')->result_array();
		$chart = [
            'labels'   => $month_dates,
            'datasets' => [
                [
        ...
        foreach ($result as $lead) {
            ...
            foreach ($chart['labels'] as $date) {
                if ($this->jdf->jdate('Y-m-d', strtotime($lead['date_converted'])) == $date) {
    ...
    {
		$monday_this_week = date('Y-m-d', strtotime('-2 day',strtotime('monday this week')));
		$sunday_this_week = date('Y-m-d', strtotime('-2 day',strtotime('sunday this week')));

        $this->db->where("(date_converted BETWEEN '$monday_this_week' and '$sunday_this_week')  AND status = 1 and lost = 0");
		// $this->db->where('CAST(date_converted as DATE) >= "' . date('Y-m-d', strtotime('saturday this week')) . '" AND CAST(date_converted as DATE) <= "' . date('Y-m-d', strtotime('friday next week')) . '" AND status = 1 and lost = 0');
        ...
        foreach ($weekly as $weekly) {
            $lead_status_day = _l(mb_strtolower('wd_' . date('l', strtotime($weekly['date_converted']))));
            $i               = 0;
    ...
    public function leads_staff_report()
    {
        ...
        if ($this->input->post()) {
            ...
            $date_date = to_sql_date_custom($this->input->post('staff_report_from_date'));
			$open_till_date   = to_sql_date_custom($this->input->post('staff_report_to_date'));
            $from_date = $date_date;
            $to_date   = $open_till_date;
    ...
    public function report_by_payment_modes()
    {
        ...
        $p_year = $this->jdf->jdate('Y' ,convert_datetime(convert_date_utc_to_local($this->input->post('year'))));
        $colors = get_system_favourite_colors();
        ...
        foreach ($data['months'] as $month) {
            array_push($chart['labels'], _l($month) . ' - ' . $p_year);
        }
        $i = 0;
    ...
    public function total_income_report()
    {
        ...
        foreach ($data['months'] as $month) {
            ...
            array_push($data['labels'], _l($month) . ' - ' . $p_year);
