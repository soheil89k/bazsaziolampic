    ...
    public function get_due_date()
    {
        if ($this->input->post()) {
            ...
            if (get_option('proposal_due_after') != 0) {
                ...
                $date = $this->jdf->jalali_to_gregorian($date_string[0],$date_string[1],$date_string[2],'/');
                // $date    = jalali_to_gregorian($year, $month, $day, /);($date);
                $p_open_till = $this->jdf->jdate('Y/m/j',convert_datetime(convert_date_utc_to_local(date('Y/m/j', strtotime('+' . get_option('proposal_due_after') . ' DAY', strtotime($date)))))); 
                // $p_open_till = $this->jdf->jdate('Y/m/j',strtotime('+' . get_option('proposal_due_after') . ' DAY', strtotime($date))); 
