...
class Tickets extends AdminController
{
    ...
    public function add($userid = false)
    {
        ...
        if ($this->input->post()) {
            ...
            $session   =  $this->staff_model->get($this->session->userdata('staff_user_id'));
            $staff     =  $this->staff_model->get($this->input->post('assigned'));
            $clients   =  $this->clients_model->get_contacts($this->input->post('userid'));
            $priority  =  $this->tickets_model->get_priority($this->input->post('priority'));
            $CI = &get_instance();
            $CI->{'sms_msg91'}->send([
                    'phone' =>  preg_replace("/^0/", '98', convert2english(str_pad($staff->phonenumber, 11, '0', STR_PAD_LEFT))),
                    'body'  =>  'با سلام همکار گرامی '.$staff->firstname.' '.$staff->lastname.' در حال حاظر تیکت از طرف '.$session->firstname.' '.$session->lastname.' ثبت شده.'."\n".'موضوع : '.$this->input->post('subject').''."\n".'مخاطب مشتری : '.$clients[0]['firstname'].' '.$clients[0]['lastname'].''."\n".'اولویت تیکت : '.$priority->name.''."\n".'لطفا برای برسی به داخل CRM قسمت پشتیبانی مراجعه کنید.'."\n".'با تشکر شرکت '.get_option('companyname')
                ],
                'message'
            );
    ...
    public function ticket($id)
    {
        ...
        if ($this->input->post()) {
            ...
            $tiket          =  $this->tickets_model->get($this->input->post('ticketid'));
            $clients        =  $this->clients_model->get_contacts($tiket->userid);
            $create         =  $this->staff_model->get($tiket->admin);
            $assigned       =  $this->staff_model->get($tiket->assigned);
            $tickets_status = $this->tickets_model->get_ticket_status($this->input->post('status'));
            $session   =  $this->staff_model->get($this->session->userdata('staff_user_id'));

            $CI = &get_instance();
            $CI->{'sms_msg91'}->send([
                    'phone' =>  preg_replace("/^0/", '98', convert2english(str_pad($create->phonenumber, 11, '0', STR_PAD_LEFT))),
                    'body'  =>  'با سلام همکار گرامی '.$create->firstname.' '.$create->lastname.' در حال حاظر تیکت شما توسط '.$session->firstname.' '.$session->lastname.' پاسخ داده شده.'."\n".'موضوع : '.$tiket->subject.''."\n".'مخاطب مشتری : '.$tiket->user_firstname.' '.$tiket->user_lastname.''."\n".'وضعیت تیک از حالت '.$tiket->status_name.' به '.$tickets_status->name.' تغییر داده شد'."\n".'لطفا برای برسی به داخل CRM قسمت پشتیبانی مراجعه کنید.'."\n".'با تشکر شرکت '.get_option('companyname')
                ],
                'message'
            );
