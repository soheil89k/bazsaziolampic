...
class Clients extends AdminController
{
    ...
    public function client($id = '')
    {
        ...
        if ($id == '') {
            ...
            $data['feedback_array'] = $this->db->where('feedback_submitted =', 1)->where('customerid', $client->userid)->get(db_prefix() . 'feedback')->result_array();
    ...
    public function form_contact($customer_id, $contact_id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if (!$this->input->post('donotsendwelcomeemail', false)) {
                $message = $this->input->post('firstname', false) . " " . $this->input->post('lastname', false) . " عزیز
                به نمایندگی از سوی کارکنان شرکت سامانه مدرن گستر فراشهر
                به شما خوش آمد گفته و از شما تقاضا میشود با پر کردن فرم زیر به آغاز فعالیت پروژه خود سرعت بخشید .
                به امید آنکه با حسن تدبیر شما و تلاش تمامی کارکنان این مجموعه، به درجات بالای موفقیت رسیده و با رفع مشکلات موجود، به عصری طلایی در کسب و کار شما قدم گذاریم." . "\n" . "https://faracity.com/crm_new/multi_page_wtl/form/5b88a2a8b2e5d7782b2a7a076189369c";

                $CI = &get_instance();
                $CI->{'sms_msg91'}->send([
                    'phone' => preg_replace("/^0/", '98', convert2english(str_pad($this->input->post('phonenumber', false), 11, '0', STR_PAD_LEFT))),
                    'body' => $message
                ],
                    'message'
                );
                ...
                $CI->{'sms_twilio'}->send(
                    str_pad($this->input->post('phonenumber', false), 11, '0', STR_PAD_LEFT),
                    $message
                );
            }
    ...
    public function zip_credit_notes($id)
    {
        ...
        if ($this->input->post()) {
            ...
            $zip_from_date = to_sql_date_custom($this->input->post('zip-from'));
            $zip_to_date = to_sql_date_custom($this->input->post('zip-to'));


            $this->load->library('app_bulk_pdf_export', [
                'export_type' => 'credit_notes',
                'status' => $this->input->post('credit_note_zip_status'),
                'date_from' => $zip_from_date,
                'date_to' => $zip_to_date,
    ...
    public function statement()
    {
        ...
        $from = to_sql_date_custom($from);
        $to = to_sql_date_custom($to);
    ...
    {
        foreach ($this->input->post('sent_to') as $contact_id) {
            if ($contact_id != '') {
                $contact = $this->clients_model->get_contact($contact_id);

                $CI = &get_instance();
                $CI->{'sms_msg91'}->send([
                    'phone' => preg_replace("/^0/", '98', convert2english(str_pad($contact->phonenumber, 11, '0', STR_PAD_LEFT))),
                    'body' => $this->input->post('email_template_custom')
                ],
                    'message'
                );

                if ($this->input->post('sms_send')) {
                    $CI->{'sms_twilio'}->send(
                        str_pad($contact->phonenumber, 11, '0', STR_PAD_LEFT),
                        $this->input->post('email_template_custom')
                    );
                }
        ...
        // $customer = $this->db->where('customer_id =', $id)->get(db_prefix() . 'customer_admins')->result_array();
        // foreach($customer as $staff) {
        //     $CI = & get_instance();
        //     $CI->db->select('phonenumber','staff_id');
        //     $CI->db->from(db_prefix() . 'staff');
        //     $CI->db->where('staffid', $staff['staff_id']);
        //     $staff = $CI->db->get()->result_array();
        //     foreach($staff[0] as $phone) {
        //         $contact = $this->clients_model->get_contact($contact_id);
        ...
        //         if($this->input->post('sms_send')) {
        //             $CI->{'sms_twilio'}->send(
        //                 str_pad($phone, 11, '0', STR_PAD_LEFT),
        //                 $this->input->post('email_template_custom')
        //             );
        //         }
        //     }
        ...
        redirect(admin_url('clients/client/' . $id . '?group=whatsapp'));
