...
class Leads extends AdminController
{
    ...
    public function add_note($rel_id)
    {
        ...
        if ($this->input->post()) {
            ...
            if ($data['contacted_indicator'] == 'yes') {
                $date_date = to_sql_date_custom($data['custom_contact_date']);
                $data['date_contacted'] = $date_date;
				$contacted_date = $date_date;
            }
            ...
            if ($note_id) {
                if (isset($contacted_date)) {
                    ...
                    if ($this->db->affected_rows() > 0) {
                        $contacted_date_full_string = explode(" ", $contacted_date);
                        $this->leads_model->log_lead_activity($rel_id, 'not_lead_activity_contacted', false, serialize([
                            get_staff_full_name(get_staff_user_id()),
                            $this->jdf->jdate('Y/m/j',convert_datetime(convert_date_utc_to_local($contacted_date_full_string[0]))).' در '.convert_digits($contacted_date_full_string[1], 'eng'),
                        ]));
                    }
                }
            }
    ...
    public function message_send($lead_id)
    {
        $contact = $this->leads_model->get($lead_id);
        $CI = &get_instance();
        $CI->{'sms_msg91'}->send([
