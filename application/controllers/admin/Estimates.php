...
class Estimates extends AdminController
{
    ...
    public function validate_estimate_number()
    {
        ...
        $date            = to_sql_date_custom($this->input->post('date'));
    ...
    public function get_due_date()
    {
        if ($this->input->post()) {
            ...
            if (get_option('estimate_due_after') != 0) {
                ...
                $p_open_till = $this->jdf->jdate('Y/m/j',convert_datetime(convert_date_utc_to_local(date('Y/m/j', strtotime('+' . get_option('estimate_due_after') . ' DAY', strtotime($date))))));
