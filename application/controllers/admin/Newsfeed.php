...
class Newsfeed extends AdminController
{
    ...
    public function load_newsfeed()
    {
        ...
        foreach ($posts as $post) {
            ...
            $response .= '<small class="text-muted">' . _l('newsfeed_published_post') . ': ' . format_to_relative_time($post['datecreated'], false) . '</small>';
            $response .= '</div>';
    ...
    public function comment_single($comment)
    {
        ...
        if (!$this->newsfeed_model->user_liked_comment($comment['id'])) {
            $_comments .= '<p class="no-margin"><a href="#" onclick="like_comment(' . $comment['id'] . ',' . $comment['postid'] . '); return false;"><small>' . _l('newsfeed_like_this_saying') . ' ' . $_comment_likes . ' - ' . format_to_relative_time($comment['dateadded'], '', false) . '</small></p>';
        } else {
            $_comments .= '<p class="no-margin"><a href="#" onclick="unlike_comment(' . $comment['id'] . ',' . $comment['postid'] . '); return false;"><small>' . _l('newsfeed_unlike_this_saying') . ' ' . $_comment_likes . ' - ' . format_to_relative_time($comment['dateadded'], '', false) . '</small></p>';
        }
        $_comments .= '</div>';
        $_comments .= '</div>';
