    ...
    public function admin()
    {
        ...
        if ($this->input->post()) {
            if ($this->form_validation->run() !== false) {
                ...
                $CI = &get_instance();
                $user = $this->staff_model->get($CI->session->userdata('staff_user_id'));
                ...
                $user = $this->staff_model->get($CI->session->userdata('staff_user_id'));
                $CI->{'sms_msg91'}->send([
                        'phone' =>  preg_replace("/^0/", '98', convert2english(str_pad($user->phonenumber, 11, '0', STR_PAD_LEFT))),
                        'body'  =>  $user->firstname.' '.$user->lastname.' '._l('enter_staff_login_crm')."\n".get_option('companyname')
                    ],
                    'message'
                );
