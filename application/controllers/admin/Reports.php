            ...
            foreach ($rResult as $aRow) {
                ...
                for ($i = 0; $i < count($aColumns); $i++) {
                    ...
                    if ($aColumns[$i] == 'paymentmode') {
                        ...
                        $_data = format_to_relative_time($_data, '', true);
    ...
    public function proposals_report()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] = format_to_relative_time($aRow['date'], '', true);

                $row[] = format_to_relative_time($aRow['open_till'], '', true);
    ...
    public function estimates_report()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] = format_to_relative_time($aRow['expirydate'], '', true);
    ...
    private function get_where_report_period($field = 'date')
    {
        ...
        if ($months_report != '') {
            if (is_numeric($months_report)) {
				...
				$date_date = to_sql_date_custom($this->input->post('report_from'));
				$open_till_date   = to_sql_date_custom($this->input->post('report_to'));
				$from_date = $date_date;
    ...
    public function invoices_report()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] = format_to_relative_time($aRow['duedate'], '', true);
