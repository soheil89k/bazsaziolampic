...
$invoice_info .= '<br />' . _l('invoice_data_date') . ' ' . to_view_date_custom($invoice->date) . '<br />';

$invoice_info = hooks()->apply_filters('invoice_pdf_header_after_date', $invoice_info, $invoice);

if (!empty($invoice->duedate)) {
    $invoice_info .= _l('invoice_data_duedate') . ' ' . to_view_date_custom($invoice->duedate) . '<br />';
...
$tbltotal = '';
$tbltotal .= '<table cellpadding="6" style="font-size:' . ($font_size + 4) . 'px" dir="rtl">';
$tbltotal .= '
<tr>
    <td align="right" width="85%"><strong>' . _l('invoice_subtotal') . '</strong></td>
    <td align="right" width="15%">' . formatPageNumber(app_format_money($invoice->subtotal, $invoice->currency_name)) . '</td>
</tr>';
...
if (is_sale_discount_applied($invoice)) {
    ...
    if (is_sale_discount($invoice, 'percent')) {
        $tbltotal .= ' (' . formatPageNumber(app_format_number($invoice->discount_percent, true)) . '%)';
    }
    $tbltotal .= '</strong>';
    $tbltotal .= '</td>';
    $tbltotal .= '<td align="right" width="15%">-' . formatPageNumber(app_format_money($invoice->discount_total, $invoice->currency_name)) . '</td>
    </tr>';
}

foreach ($items->taxes() as $tax) {
    $tbltotal .= '<tr>
    <td align="right" width="85%"><strong>' . formatPageNumber($tax['taxname']) . ' (' . formatPageNumber(app_format_number($tax['taxrate'])) . '%)' . '</strong></td>
    <td align="right" width="15%">' . formatPageNumber(app_format_money($tax['total_tax'], $invoice->currency_name)) . '</td>
</tr>';
}
...
<tr style="background-color:#f0f0f0;">
    ...
    <td align="right" width="15%">' . formatPageNumber(app_format_money($invoice->total, $invoice->currency_name)) . '</td>
</tr>';
...
if (count($invoice->payments) > 0 && get_option('show_total_paid_on_invoice') == 1) {
    ...
    <tr>
        ...
        <td align="right" width="15%" style="color:green;">-' . formatPageNumber(app_format_money(sum_from_table(db_prefix().'invoicepaymentrecords', [
        'field' => 'amount',
        'where' => [
...
if (get_option('show_credits_applied_on_invoice') == 1 && $credits_applied = total_credits_applied_to_invoice($invoice->id)) {
    ...
    <tr>
        ...
        <td align="right" width="15%">-' . formatPageNumber(app_format_money($credits_applied, $invoice->currency_name)) . '</td>
    </tr>';
}
...
if (get_option('show_amount_due_on_invoice') == 1 && $invoice->status != Invoices_model::STATUS_CANCELLED) {
    $tbltotal .= '<tr style="background-color:#f0f0f0;">
       ...
       <td align="right" width="15%" style="color:red;" dir="rlt">' . formatPageNumber(app_format_money($invoice->total_left_to_pay, '')).'</td>
   </tr>';
}
...
if (count($invoice->payments) > 0 && get_option('show_transactions_on_invoice_pdf') == 1) {
    ...
    $pdf->SetFont($font_name, 'B', $font_size);
    $pdf->Cell(0, 0, _l('invoice_received_payments') . ":", 0, 1, 'R', 0, '', 0);
    $pdf->SetFont($font_name, '', $font_size);
    $pdf->Ln(4);
    $tblhtml = '<table width="100%" bgcolor="#fff" cellspacing="0" cellpadding="5" border="0">
        ...
        <th width="25%;" style="' . $border . '">' . _l('invoice_payments_table_amount_heading').' ('.$invoice->symbol.')</th>
    </tr>';
    $tblhtml .= '<tbody>';
    foreach ($invoice->payments as $payment) {
        ...
        $tblhtml .= '
            <tr>
            <td>' . formatPageNumber($payment['paymentid']) . '</td>
            <td>' . $payment_name . '</td>
            <td>' . to_view_date_custom($payment['date']) . '</td>
            <td>' . formatPageNumber(app_format_money($payment['amount'], '')) . '</td>
            </tr>
        ';
    }
    $tblhtml .= '</tbody>';
