...
foreach ($rResult as $aRow) {
    ...
    $row[] = ($aRow['lastcontact'] == '0000-00-00 00:00:00' || !is_date($aRow['lastcontact']) ? '' : '<span data-toggle="tooltip" data-title="' . format_to_relative_time($aRow['lastcontact'], false) . '" class="text-has-action is-date">' . time_ago($aRow['lastcontact']) . '</span>');

    $row[] = '<span data-toggle="tooltip" data-title="' . format_to_relative_time($aRow['dateadded'], false) . '" class="text-has-action is-date">' . time_ago($aRow['dateadded']) . '</span>';
