...
if ($filter != 'period') {
    ...
    $date_date_pure = $this->ci->input->post('period-from');
    $open_till_date_pure = $this->ci->input->post('period-to');
    $is_date_english_date = substr_count($date_date_pure, '-');
    $is_open_till_english_date = substr_count($open_till_date_pure, '-');
    if(!$is_date_english_date){
            $date_date_string = explode("/", $this->ci->input->post('period-from'));
            $date_date = $this->ci->jdf->jalali_to_gregorian($date_date_string[0],$date_date_string[1],$date_date_string[2],'-');
    } else {
            $date_date = $this->ci->input->post('period-from');
    }
    if(!$is_open_till_english_date && $this->ci->input->post('period-to')){
            $open_till_date_string = explode("/", $this->ci->input->post('period-to'));
            $open_till_date = $this->ci->jdf->jalali_to_gregorian($open_till_date_string[0],$open_till_date_string[1],$open_till_date_string[2],'-');
    } else {
            $open_till_date = $this->ci->input->post('period-to');
    }

    $start_date = $date_date;
    $end_date   = $open_till_date;
...
foreach ($rResult as $aRow) {
    ...
    if (! $roundTimesheets) {
        $row[] = format_to_relative_time(_dt($aRow['start_time'], true), false);
        $row[] = ($aRow['end_time'] ? format_to_relative_time(_dt($aRow['end_time'], true), false) : '');
    }
