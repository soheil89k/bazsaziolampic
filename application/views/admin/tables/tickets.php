    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == '1') {
            ...
            if ($aRow[$aColumns[$i]] == null) {
                ...
                $_data = format_to_relative_time($aRow[$aColumns[$i]], false);
            }
            ...
            $_data = format_to_relative_time($_data, false);
        } elseif ($aColumns[$i] == 'priority') {
            $_data = ticket_priority_translate($aRow['priority']);
        } else {
            if (strpos($aColumns[$i], 'date_picker_') !== false) {
                $_data = (strpos($_data, ' ') !== false ? format_to_relative_time($_data, false) : format_to_relative_time($_data, '', true));
            }
        }
