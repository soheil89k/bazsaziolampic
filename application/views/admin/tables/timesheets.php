    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($i == 0) {
            ...
            if ($aColumns[$i] == 'end_time' && $_data == null) {
                ...
                $_data = format_to_relative_time(_dt($_data, true), false);
            }
        } elseif ($i == 2) {
    ...
    if (($aRow['staff_id'] == get_staff_user_id() || has_permission('projects', '', 'edit'))) {
        if (($aRow['staff_id'] == get_staff_user_id() || has_permission('projects', '', 'edit'))) {
            if ($aRow['end_time'] !== null) {
                $persian_date_time = explode(' ', _dt($aRow['start_time'], true));
                $p_date = $this->ci->jdf->jdate('Y/m/j',convert_datetime(convert_date_utc_to_local($persian_date_time[0]))).' '.$persian_date_time[1];
                $attrs = [
                'onclick'                 => 'edit_timesheet(this,' . $aRow['id'] . ');return false',
                'data-start_time'         => $p_date,
                ...
                $persian_end_date_time = explode(' ', _dt($aRow['end_time'], true));
                $p_end_date = $this->ci->jdf->jdate('Y/m/j',convert_datetime(convert_date_utc_to_local($persian_end_date_time[0]))).' '.$persian_end_date_time[1];

                $attrs['data-end_time'] = $p_end_date;
