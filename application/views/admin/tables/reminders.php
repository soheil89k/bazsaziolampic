...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'staff') {
            ...
            $date_full_string = explode(" ", $_data);
            $date_date_string = format_to_relative_time($date_full_string[0], '', true);
            $_data = $date_date_string.' در '.convert_digits($date_full_string[1], 'persian');
        }
        $row[] = $_data;
    }
