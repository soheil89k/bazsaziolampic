   ...
   <div class="row">
      <div class="lead-view<?php if(!isset($lead)){echo ' hide';} ?>" id="leadViewWrapper">
         ...
         <div class="col-md-4 col-xs-12 lead-information-col">
            ...
            <p class="bold font-medium-xs"><?php echo (isset($lead) && $lead->dateadded != '' ? '<span class="text-has-action" data-toggle="tooltip" data-title="'.format_to_relative_time($lead->dateadded, false).'">' . time_ago($lead->dateadded) .'</span>' : '-') ?></p>
            <p class="text-muted lead-field-heading"><?php echo _l('leads_dt_last_contact'); ?></p>
            <p class="bold font-medium-xs"><?php echo (isset($lead) && $lead->lastcontact != '' ? '<span class="text-has-action" data-toggle="tooltip" data-title="'.format_to_relative_time($lead->lastcontact, false).'">' . time_ago($lead->lastcontact) .'</span>' : '-') ?></p>
         ...
         <div class="col-md-12">
            ...
            <div class="row">
               <div class="col-md-12">
                  ...
                  <?php } else { ?>
                     <?php
                        $p_date = to_view_date_custom($lead->lastcontact);
                        echo render_datetime_input('lastcontact','leads_dt_last_contact',$p_date.' '.convert_digits(explode(' ', $lead->lastcontact)[1], 'persian'),array('data-date-end-date'=>date('Y-m-d'))); ?>
                  <?php } ?>
