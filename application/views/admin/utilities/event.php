    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="row">
          <div class="col-md-12">
            ...
            <?php if($event->userid == get_staff_user_id() || is_admin()){ ?>
              ...
              <?php echo render_datetime_input('start','utility_calendar_new_event_start_date',$this->jdf->jdate('Y/m/j H:i',convert_datetime(($event->start)))); ?>
              <div class="clearfix mtop15"></div>
              <?php
			  $value = (isset($task) ? _d($task->duedate) : '');
			   echo render_datetime_input('end','utility_calendar_new_event_end_date',$event->end ? to_view_date_custom($event->end, 'Y/m/j H:i') : '' )?>
        ...
        <?php } else { ?>
          ...
          <p><?php echo $this->jdf->jdate('Y/m/j H:i',convert_datetime(convert_date_utc_to_local($event->start))); ?></p>
          <?php if(is_date($event->end)){ ?>
            <h5 class="bold"><?php echo _l('utility_calendar_new_event_end_date'); ?></h5>
            <p><?php echo $this->jdf->jdate('Y/m/j H:i',convert_datetime(convert_date_utc_to_local($event->end))); ?></p>
          <?php } ?>
        <?php } ?>
      </div>
    </div>
