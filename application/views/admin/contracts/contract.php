         ...
         <div class="row">
            <div class="col-md-6">
               <?php $value = (isset($contract) ? _d($contract->datestart) : _d(date('Y-m-d')));
                $p_start_date = isset($contract)? to_view_date_custom($contract->datestart) : to_view_date_custom(date('Y-m-d')); 
               ?>
               <?php echo render_date_input('datestart','contract_start_date',$p_start_date); ?>
            </div>
            <div class="col-md-6">
               <?php $value = (isset($contract) ? _d($contract->dateend) : '');
                $p_end_date = isset($contract)? to_view_date_custom($contract->dateend) : ''; 
               ?>
               <?php echo render_date_input('dateend','contract_end_date',$p_end_date); ?>
            </div>
         </div>
            ...
            <div class="tab-content">
               ...
               <div class="tc-content<?php if(staff_can('edit','contracts')){echo ' editable';} ?>"
                  ...
                  if(empty($contract->content) && staff_can('edit','contracts')){
                    echo hooks()->apply_filters('new_contract_default_content', '<span class="text-danger text-uppercase mtop15 editor-add-content-notice"> ' . $template->message . '</span>');
                 } else {
      ...
      <div class="media-body">
         <div class="display-block">
            ...
            <?php if($renewal['renewed_by_staff_id'] == get_staff_user_id() || is_admin()){ ?>
               ...
               <br />
            <?php } ?>
            <small class="text-muted"><?php echo format_to_relative_time($renewal['date_renewed'], false); ?></small>
            <hr class="hr-10" />
            <span class="text-success bold" data-toggle="tooltip" title="<?php echo _l('contract_renewal_old_start_date',format_to_relative_time($renewal['old_start_date'], '', true)); ?>">
               <?php echo _l('contract_renewal_new_start_date',format_to_relative_time($renewal['new_start_date'], '', true)); ?>
            </span>
            <br />
            <?php if(is_date($renewal['new_end_date'])){
               $tooltip = '';
               if(is_date($renewal['old_end_date'])){
                 $tooltip = _l('contract_renewal_old_end_date',format_to_relative_time($renewal['old_end_date'], '', true));
              }
              ?>
              <span class="text-success bold" data-toggle="tooltip" title="<?php echo $tooltip; ?>">
               <?php echo _l('contract_renewal_new_end_date',format_to_relative_time($renewal['new_end_date'], '', true)); ?>
            </span>
            <br/>
         <?php } ?>
