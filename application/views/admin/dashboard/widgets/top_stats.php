...
<div class="widget relative" id="widget-<?php echo create_widget_id(); ?>" data-name="<?php echo _l('quick_stats'); ?>">
    ...
    <div class="row">
        ...
        <div class="quick-stats-invoices col-xs-12 col-md-6 col-sm-6">
            <div class="top_stats_wrapper">
                <p class="text-uppercase mtop5"><i
                            class="hidden-sm fa fa-envelope-open-o"></i> <?php echo _l('sms_count'); ?>
                    <span class="pull-right">
                     ...
                     try {
                         ...
                         echo $req . ' ریال';
                         $count = (int)(str_replace(",", "", $req) / ('5000000' - str_replace(",", "", $req)) * 100);
                     } catch (Exception $e) {
                         echo "اتصال موقت قطع است";
                     }
                     ?>
                  </span>
                </p>
                ...
                <div class="progress no-margin progress-bar-mini">
                    <div class="progress-bar progress-bar-success no-percent-text not-dynamic" role="progressbar"
                         aria-valuemin="0" aria-valuemax="100" aria-valuenow="<?php echo $count; ?>"
                         data-percent="<?php echo $count; ?>" style="width: 0%">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="quick-stats-invoices col-xs-12 col-md-6 col-sm-6">
            <div class="top_stats_wrapper">
                <p class="text-uppercase mtop5"><i class="hidden-sm fa fa-calendar-times-o"></i>
                    <!-- <?php /*echo get_date_difference_in_days(date("Y-m-d H:i:s"), check_footer_load()['request']['expire']) */ ?>
                    --><?php /*echo _l('lisenes_time'); */ ?>
                    <!-- <span class="pull-right"><?php /*echo to_view_date_custom(check_footer_load()['request']['expire']);
                        $date = date('y-m-j&\nb\sp;g:i:s');
                        $date1 = strtotime(check_footer_load()['request']['expire']);
                        $date2 = strtotime(check_footer_load()['request']['created_at']);
                        $today = time();
                ...
                <div class="progress no-margin progress-bar-mini">
                    <!-- <div class="progress-bar progress-bar-success no-percent-text not-dynamic" role="progressbar"
                         aria-valuemin="0" aria-valuemax="100" aria-valuenow="<?php /*echo $percentageRounded; */ ?>"
                         data-percent="<?php /*echo $percentageRounded; */ ?>" style="width: 0%">
                    </div>-->
                </div>
            </div>
        </div>
        ...
        <div class="quick-stats-projects col-xs-12 col-md-6 col-sm-6 <?php echo $initial_column; ?>">
            <div class="top_stats_wrapper">
                ...
                ?>
                <p class="text-uppercase mtop5"><i
                            class="hidden-sm fa fa-cubes"></i> <?php echo _l('projects') . ' ' . $project_status['name']; ?>
                    <span class="pull-right"><?php echo $total_projects_in_progress; ?> / <?php echo $total_projects; ?></span>
                </p>
                <div class="clearfix"></div>
                <div class="progress no-margin progress-bar-mini">
