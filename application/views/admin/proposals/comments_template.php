...
<div class="col-md-12" data-commentid="<?php echo $comment['id']; ?>">
   ...
   <div class="media-body">
      <div class="mtop5">
      <?php if($comment['staffid'] != 0){ ?>
         <a href="<?php echo admin_url('profile/' . $comment['staffid']); ?>" style="display:inline-block"><?php echo get_staff_full_name($comment['staffid']); ?></a>
      <?php } else { ?>
         <?php echo '<b>' . _l('is_customer_indicator') . '</b>'; ?>
      <?php } ?>
         <small class="text-muted text-has-action" data-toggle="tooltip" data-title="<?php echo format_to_relative_time($comment['dateadded'],'', false); ?>"> - <?php echo time_ago($comment['dateadded']); ?></small>
      </div>
      <br />
