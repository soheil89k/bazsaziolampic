         ...
         <div class="col-md-6">
            ...
            <div class="row">
               <div class="col-md-6">
                  ...
                  $p_date = isset($invoice)? to_view_date_custom($invoice->date) : to_view_date_custom(date('Y-m-d')); 
                  ?>
                  <?php echo render_date_input('date','invoice_add_edit_date',$p_date,$date_attrs); ?>
               </div>
               <div class="col-md-6">
                  <?php
                  $value = '';
                  if(isset($invoice)){
                    ...
                    $p_duedate = to_view_date_custom($invoice->duedate);
                  } else {
                    if(get_option('invoice_due_after') != 0){
                        $value = _d(date('Y-m-d', strtotime('+' . get_option('invoice_due_after') . ' DAY', strtotime(date('Y-m-d')))));
                        $p_duedate = to_view_date_custom(date('Y-m-d', strtotime('+' . get_option('invoice_due_after') . ' DAY', strtotime(date('Y-m-d'))))); 
                    }
                  }
                   ?>
                  <?php echo render_date_input('duedate','invoice_add_edit_duedate',$p_duedate); ?>
               </div>
            </div>
