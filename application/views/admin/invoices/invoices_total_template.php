...
<div class="row">
   ...
   <div class="col-md-12 simple-bootstrap-select mbot5">
      ...
      <?php } ?>
      <?php if(count($invoices_p_years) > 1){ ?>
      <select data-none-selected-text="<?php echo $this->jdf->jdate('Y'); ?>" data-width="auto" class="selectpicker" name="invoices_total_years" onchange="init_invoices_total();" multiple="true" id="invoices_total_years">
         <?php
        foreach($invoices_p_years as $year){?>
         <option value="<?php echo $year; ?>"<?php if($this->input->post('years') && $year == $this->input->post('years') || !$this->input->post('years') && $this->jdf->jdate('Y') == $year){echo ' selected'; } ?>><?php echo $year; ?></option>
         <?php } ?>
      </select>
      <?php } ?>
   </div>
