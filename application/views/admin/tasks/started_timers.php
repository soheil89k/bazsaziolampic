...
foreach ($startedTimers as $timer) {
    ...
    $data .= '<span class="text-muted">' . _l('timer_top_started', format_to_relative_time(_dt($timer['start_time'], true), '', false)) . '</span><br /><span class="text-success">'._l('task_total_logged_time') .' '. seconds_to_time_format($this->tasks_model->calc_task_total_time($timer['task_id'], ' AND staff_id='.get_staff_user_id())).'</span>';
