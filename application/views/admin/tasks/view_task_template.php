   ...
   <div class="row">
               ...
               <table class="table">
                  ...
                  <tbody>
                     ...
                     <tr>
                        ...
                        <td><?php
                         $start_full_string = explode(" ", _dt($timesheet['start_time'], true));
                         $start_date_string = format_to_relative_time($start_full_string[0], '', true);
                         echo $start_date_string.' '.convert_digits($start_full_string[1], 'persian'); ?>
                         </td>
                        <td>
                           <?php
                              if($timesheet['end_time'] !== NULL){
                                 $end_full_string = explode(" ", _dt($timesheet['end_time'],true));
                                 $end_date_string = format_to_relative_time($end_full_string[0], '', true);
                                 echo $end_date_string.' '.convert_digits($end_full_string[1], 'persian');
                              } else {
                        ...
                        <li class="mbot10 task-attachment<?php if(strtotime($attachment['dateadded']) >= strtotime('-16 hours')){echo ' highlight-bg'; } ?>">
                           <div class="mbot10 pull-right task-attachment-user">
                              ...
                              <?php }
                                 ...
                                 echo '<span class="text-has-action" data-toggle="tooltip" data-title="'.format_to_relative_time($attachment['dateadded'], '', false).'">'.time_ago($attachment['dateadded']).'</span>';
                                 ?>
                           </div>
                           <div class="clearfix"></div>
            ...
            <div id="task-comments" class="mtop10">
               <?php
                  ...
                  foreach ($task->comments as $comment) {
                    ...
                    $comments .= '<a data-task-comment-href-id="'.$comment['id'].'" href="'.admin_url('tasks/view/'.$task->id).'#comment_'.$comment['id'].'" class="task-date-as-comment-id"><small><span class="text-has-action inline-block mbot5" data-toggle="tooltip" data-title="'.format_to_relative_time($comment['dateadded'], '', false).'">' . time_ago($comment['dateadded']) . '</span></small></a>';
      ...
      <div class="col-md-4 task-single-col-right">
         ...
         <h5 class="no-mtop task-info-created">
            ...
            <small class="text-dark"><?php echo _l('task_created_by','<span class="text-dark">'.($task->is_added_from_contact == 0 ? get_staff_full_name($task->addedfrom) : get_contact_full_name($task->addedfrom)).'</span>'); ?> <i class="fa fa-clock-o" data-toggle="tooltip" data-title="<?php echo _l('task_created_at',format_to_relative_time($task->dateadded, false)); ?>"></i></small>
            <br />
            <?php } else { ?>
            <small class="text-dark"><?php echo _l('task_created_at','<span class="text-dark">'.format_to_relative_time($task->dateadded, false).'</span>'); ?></small>
            <?php } ?>
         </h5>
         ...
         <div class="task-info task-single-inline-wrap task-info-start-date">
            <h5><i class="fa task-info-icon fa-fw fa-lg fa-calendar-plus-o pull-left fa-margin"></i>
               ...
               <?php if(has_permission('tasks','','edit') && $task->status !=5) { 
                  $p_date = to_view_date_custom($task->startdate);  
                  ?>
               <input name="startdate" tabindex="-1" data-targetselector="#task-single-startdate" value="<?php echo $p_date; ?>" id="task-single-startdate" class="task-info-inline-input-edit datepicker pointer task-single-inline-field">
               <?php } else { ?>
               <?php echo format_to_relative_time($task->startdate, '', true); ?>
               <?php } ?>
            </h5>
         </div>
         <div class="task-info task-info-due-date task-single-inline-wrap<?php if(!$task->duedate && !has_permission('edit','','tasks')){echo ' hide';} ?>"<?php if(!$task->duedate){ echo ' style="opacity:0.5;"';} ?>>
            <h5>
               ...
               <?php if(has_permission('tasks','','edit') && $task->status !=5) { 
                  $p_date = to_view_date_custom($task->duedate);
                  ?>
               <input
               name="duedate" data-targetselector="#task-single-duedate"
               tabindex="-1"
               ...
               <?php } else { ?>
               <?php echo format_to_relative_time($task->duedate, '', true); ?>
               <?php } ?>
            </h5>
         </div>
            ...
            <li class="<?php if($reminder['isnotified'] == '1'){echo 'text-throught';} ?>" data-id="<?php echo $reminder['id']; ?>">
               <div class="mbot15">
                  <div>
                     <p class="bold">
                        <?php
                        $start_full_string = explode(" ", _dt($reminder['date']));
                        $start_date_string = format_to_relative_time($start_full_string[0], '', true);
                        $r_date = $start_date_string.' '.convert_digits($start_full_string[1], 'persian');
