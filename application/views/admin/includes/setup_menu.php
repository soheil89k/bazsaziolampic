...
<div id="setup-menu-wrapper" class="animated <?php if ($this->session->has_userdata('setup-menu-open')
    ...
    <ul class="nav metis-menu" id="setup-menu">
                ...
                <?php if (count($item['children']) > 0) { ?>
                    <ul class="nav nav-second-level collapse" aria-expanded="false">
                        <?php foreach ($item['children'] as $submenu) {
                            ?>
                            <li class="sub-menu-item-<?php echo $submenu['slug']; ?>"><a
                                        href="<?php echo $submenu['href']; ?>">
        ...
        <li class="<?php echo $_SERVER[REQUEST_URI] == '/admin/setting/message' ? 'active' : '' ?>">
        ...
        <?php if (get_option('show_help_on_setup_menu') == 1 && is_admin()) {
            $totalSetupMenuItems++; ?>
            <li>
                <a href="<?php echo hooks()->apply_filters('help_menu_item_link', 'https://pegus.ir'); ?>"
                   target="_blank">
