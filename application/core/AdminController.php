...
class AdminController extends App_Controller
{
    public function __construct()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            if (ENVIRONMENT == 'production' && is_admin()) {
                if ($this->config->item('encryption_key') === '') {
                    die('<h1>Encryption key not sent in application/config/app-config.php</h1>For more info visit <a href="https://pegus.ir">encryption key explained</a>');
                } elseif (strlen($this->config->item('encryption_key')) != 32) {
                    die('<h1>Encryption key length should be 32 charachters</h1>For more info visit <a href="https://pegus.ir">encryption key explained</a>');
                }
            }
        ...
        hooks()->do_action('admin_init',function (){
