...
function to_sql_date_custom($date, $digits = false)
{
    $CI = &get_instance();
    if ($date == "" || $date == NULL) {
        return NULL;
    }
    $is_english_date = substr_count($date, "-");
    if ($digits) {
        if (!$is_english_date) {
            $date_full_string = explode(" ", $date);
            $date_string = explode("/", $date_full_string[0]);
            $date = $CI->jdf->jalali_to_gregorian($date_string[0], $date_string[1], $date_string[2], "-") . " " . convert_digits($date_full_string[1], "eng");
        } else {
            $date = $date;
        }
    } else {
        if (!$is_english_date) {
            ...
            $date = $CI->jdf->jalali_to_gregorian($date_string[0], $date_string[1], $date_string[2], "-");
        } else {
            $date = $date;
        }
    }
...
function to_view_date_custom($date, $format = "Y/m/j")
{
    if ($date == "" || $date == NULL) {
        return NULL;
    }
    $CI = &get_instance();
    $date = $CI->jdf->jdate($format, convert_datetime(convert_date_utc_to_local($date)));
    return $date;
}

function to_view_date_time_custom($date, $format = "Y/m/j H:i:s")
{
    if ($date == "" || $date == NULL) {
        return NULL;
    }
