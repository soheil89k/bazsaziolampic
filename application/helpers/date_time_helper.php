...
if (!function_exists('get_timezone_offset')) {
    ...
    function get_timezone_offset() {
        $timeZone = new DateTimeZone(get_option("timezone"));
...
if (!function_exists('convert_date_utc_to_local')) {
    ...
    function convert_date_utc_to_local($date_time, $format = "Y-m-d H:i:s") {
        ...
        $date->setTimezone(new DateTimeZone(get_option('default_timezone')));
...
if (!function_exists('convert_time_to_12hours_format')) {
    ...
    function convert_time_to_12hours_format($time = "") {
        ...
        //     $am = " ق.ظ";
        //     $pm = " ب.ظ";
        //     if (get_option("time_format") === "small") {
        //         $am = " am";
        //         $pm = " pm";
        //     }
        //     $check_time = explode(":", $time);
...
if (!function_exists('format_to_relative_time')) {
    ...
    function format_to_relative_time($date_time, $convert_to_local = true, $is_short_date = false) {
        ...
        $ci->load->library("Jdf");
        $formatted = "";
        if ($date_time == "" || is_null($date_time) || $date_time == "0000-00-00") {
            return $formatted;
        }
        if ($convert_to_local) {
        ...
        $now = new DateTime();
        $now->setTimezone(new DateTimeZone(get_option("default_timezone")));
        ...
        if ($now->format("Y-m-d") == $target_date->format("Y-m-d")) {
            ...
            if (date("Y-m-d", strtotime(" -1 day", strtotime($today))) === $target_date->format("Y-m-d")) {
                $date = _l("yesterday_at");
                $short_date = _l("yesterday");
            } else {
                $date = $ci->jdf->jdate("l, j F Y", convert_datetime(convert_date_utc_to_local($date_time)));
                $short_date = $ci->jdf->jdate("j F Y", convert_datetime(convert_date_utc_to_local($date_time)));
            }
        }
        if ($is_short_date) {
            return $short_date;
        }
        if (get_option("time_format") == "24_hours") {
...
if (!function_exists('format_to_date')) {
    ...
    function format_to_date($date_time, $convert_to_local = true) {
        ...
        $target_date = new DateTime($date_time);
        return $target_date->format(get_option('default_timezone'));
    }
...
if (!function_exists('format_to_datetime')) {
    ...
    function format_to_datetime($date_time, $convert_to_local = true) {
        ...
        $target_date = new DateTime($date_time);
        $date = $target_date->format(get_option('date_format'));
