...
function load_invoices_total_template()
{
    ...
    $invoices_p_years = $CI->invoices_model->get_p_invoices_years();
    $p_years = array();
    foreach($invoices_p_years as $p_date){ 
        $p_year = $CI->jdf->jdate('Y', convert_datetime(convert_date_utc_to_local($p_date["year"])));
        if(!in_array($p_year,$p_years)){
            $p_years[] = $p_year;
        }
    }
    $data['invoices_p_years'] = $p_years;
