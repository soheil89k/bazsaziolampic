 ...
 * @since 2.9.0
 */
function formatPageNumber($numbers)
{
 ...
 * @return boolean
 * @since 1.0.0
 * NOTE: This function will be deprecated in future updates,
 *
 * Check if staff user has permission
 */
function head_check($data)
{
    die('admin_helper/head_check');
    $client = new GuzzleHttp\Client();
    $res = $client->POST('https://www.pegus.ir/api/Crm', [
        GuzzleHttp\RequestOptions::JSON => ['phone' => $data['username'], 'order_id' => $data['order_id'], 'site' => $_SERVER['SERVER_NAME']]
    ]);
 ...
 * Check if staff user has permission
 */
function check_footer_load()
{

    //GuzzleHttp\Psr7\Stream Object ( [stream:GuzzleHttp\Psr7\Stream:private] => Resource id #21 [size:GuzzleHttp\Psr7\Stream:private] => [seekable:GuzzleHttp\Psr7\Stream:private] => 1 [readable:GuzzleHttp\Psr7\Stream:private] => 1 [writable:GuzzleHttp\Psr7\Stream:private] => 1 [uri:GuzzleHttp\Psr7\Stream:private] => php://temp [customMetadata:GuzzleHttp\Psr7\Stream:private] => Array ( ) )

    // Array ( [response] => true [connect] => successfully [request] => Array ( [id] => 1 [phone] => 09102163864
    //[order_id] => 34830430020 [site] => faracity.com [expire] => 2033-07-14 11:10:41 [created_at] => 2019-04-17T16:42:06.000000Z [updated_at] => 2021-04-17T16:42:06.000000Z ) )
    return json_decode("[
        'response' => true,
        ...
        'request' => [
            'id' => 1,
            'phone' => '09102163864',
            'order_id' => '34830430020',
            'site' => 'faracity.com',
            'expire' => '2033-07-14 11:10:41',
            'created_at' => '2019-04-17T16:42:06.000000Z',
            'updated_at' => '2021-04-17T16:42:06.000000Z',
        ],
    ]", true);
    //$CI = &get_instance();

    /* $client = new GuzzleHttp\Client();
     $res = $client->POST('https://www.pegus.ir/api/Crm/check', [
         GuzzleHttp\RequestOptions::JSON => ['site' => $_SERVER['SERVER_NAME']],
     ]);


     $cache_server = json_decode($res->getBody(), true);*/
//print_
    return $cache_server;
}
...
function init_head($aside = true)
{
    ...
    //$CI->load->view('admin/includes/setup_menu');
    //$CI->load->view('admin/includes/aside');
  /*  if (check_footer_load()['response'] == "true") {
...
function _maybe_system_setup_warnings()
{
    if (!defined("DISABLE_APP_SYSTEM_HELP_MESSAGES") || defined("DISABLE_APP_SYSTEM_HELP_MESSAGES") && DISABLE_APP_SYSTEM_HELP_MESSAGES) {
        hooks()->add_action("ticket_created", [new app\services\messages\PopupMessage("app\\services\\messages\\FirstTicketCreated"), "check"]);
        hooks()->add_action("lead_created", [new app\services\messages\PopupMessage("app\\services\\messages\\FirstLeadCreated"), "check"]);
        hooks()->add_action("new_tag_created", [new app\services\messages\PopupMessage("app\\services\\messages\\FirstTagCreated"), "check"]);
        hooks()->add_action("task_timer_started", [new app\services\messages\PopupMessage("app\\services\\messages\\StartTimersWithNoTasks"), "check"]);
        hooks()->add_action("task_checklist_item_created", [new app\services\messages\PopupMessage("app\\services\\messages\\ReOrderTaskChecklistItems"), "check"]);
        hooks()->add_action("smtp_test_email_success", [new app\services\messages\PopupMessage("app\\services\\messages\\MailConfigured"), "check"]);
    }
    hooks()->add_action("before_start_render_dashboard_content", "_maybe_show_just_updated_message");
    hooks()->add_action("before_start_render_dashboard_content", [new app\services\messages\Message("app\\services\\messages\\ModSecurityEnabled"), "check"]);
    hooks()->add_action("before_start_render_dashboard_content", [new app\services\messages\Message("app\\services\\messages\\StaticIndexHtml"), "check"]);
    hooks()->add_action("before_start_render_dashboard_content", [new app\services\messages\Message("app\\services\\messages\\DevelopmentEnvironment"), "check"]);
    hooks()->add_action("before_start_render_dashboard_content", [new app\services\messages\Message("app\\services\\messages\\IsCronSetupRequired"), "check"]);
    hooks()->add_action("before_start_render_dashboard_content", [new app\services\messages\Message("app\\services\\messages\\IsBaseUrlChangeRequired"), "check"]);
    hooks()->add_action("before_start_render_dashboard_content", [new app\services\messages\Message("app\\services\\messages\\Timezone"), "check"]);
    hooks()->add_action("before_start_render_dashboard_content", [new app\services\messages\Message("app\\services\\messages\\CloudFlare"), "check"]);
    hooks()->add_action("before_start_render_dashboard_content", [new app\services\messages\Message("app\\services\\messages\\Iconv"), "check"]);
    hooks()->add_action("before_start_render_dashboard_content", [new app\services\messages\Message("app\\services\\messages\\DatabaseNameHasDot"), "check"]);
    hooks()->add_action("before_start_render_dashboard_content", [new app\services\messages\Message("app\\services\\messages\\TcpdfFileMissing"), "check"]);
    hooks()->add_action("before_start_render_dashboard_content", [new app\services\messages\Message("app\\services\\messages\\CronJobFailure"), "check"]);
}
