...
class Commission extends AdminController
{
	...
	public function update_commission_policy($id){
        ...
        if ($this->input->post()) {
            $data = $this->input->post();
            $data['from_date'] = to_sql_date_custom($this->input->post('from_date'));
            $data['to_date'] = to_sql_date_custom($this->input->post('to_date'));
    ...
    public function commission_table()
    {
        ...
        'LEFT JOIN ' . db_prefix() . 'clients ON ' . db_prefix() . 'clients.userid = ' . db_prefix() . 'invoices.clientid',];
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] = to_view_date_custom($aRow['commission_date']);
    ...
    public function commission_receipt_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] = app_format_money($aRow['amount'], $currency->name);;
                $row[] = to_view_date_custom($aRow['date']);
