	...
	*/
	public function add_work_shift($data){
		...
		if(!$this->check_format_date_ymd($data['from_date'])){
			$data_insert['from_date'] = to_sql_date_custom($data['from_date']);
		}
		else{
			$data_insert['from_date'] = $data['from_date'];
		}
		if(!$this->check_format_date_ymd($data['to_date'])){
			$data_insert['to_date'] = to_sql_date_custom($data['to_date']);
		}
		else{
...
public function delete_shift($id){
	...
	public function add_requisition_ajax($data){
		...
		if(isset($data['request_date'])){
			$request_date = to_sql_date_custom($data['request_date']);
		...
		if($check_proccess){
			...
			if($insert_id){
				...
				return redirect(admin_url('timesheets/requisition_manage'));
			}else{
				return false;
			}
		}else{
	...
	public function report_by_leave_statistics()
	{
		...
		if ($months_report != '') {
			...
			if (is_numeric($months_report)) {
				...
				$from_date = to_sql_date_custom($this->input->post('report_from'));
				$to_date   = to_sql_date_custom($this->input->post('report_to'));
	...
	public function add_shift_type($data){      
		if(isset($data['time_start'])){
			if(!$this->check_format_date_ymd($data['time_start'])){
				$data['time_start'] = to_sql_date_custom($data['time_start']);
			}
		}
		if(isset($data['time_end'])){        
			if(!$this->check_format_date_ymd($data['time_end'])){
				$data['time_end'] = to_sql_date_custom($data['time_end']);
			}              
		}              
					...
					if($data['type_check'] == 1){
						foreach ($staff_array_id as $key => $staffid) {
							...
							if($email != ''){
								...
								$data_send_mail['date_time'] = to_view_date_custom($data['date']);
								$template = mail_template('attendance_notice', 'timesheets', array_to_object($data_send_mail));
								$template->send();
								$this->notifications($staffid, 'timesheets/requisition_manage', 'checked in at '.to_view_date_custom($data['date']));
							}
						}
					}
					else{
						foreach ($staff_array_id as $key => $staffid) {
							...
							if($email != ''){
								...
								$template = mail_template('attendance_notice', 'timesheets', array_to_object($data_send_mail));
								$template->send();
								$this->notifications($staffid, 'timesheets/requisition_manage', 'checked out at '.to_view_date_custom($data['date']));
							}
						}
					}
				}
...
public function format_date_time($date){
	if(!$this->check_format_date($date)){
		$date = to_sql_date_custom($date, true);
	}
	return $date;
}
	/**
...
public function get_timesheet($staffid='', $from_date, $to_date){
	...
	public function add_additional_timesheets($data, $staffid = ''){
		...
		$data['additional_day'] = to_sql_date_custom($data['additional_day']);
		$data['status'] = '0';
...
public function delete_additional_timesheets($id){
	...
	public function advance_payment_update($id, $data){
		...
		$data['received_date'] = to_sql_date_custom($data['received_date']);
		$this->db->where('id',$id);
