...
public function add_requisition_ajax(){
	...
	if ($this->input->post()) {
		...
		$staff_request = $this->staff_model->get(get_staff_user_id());
		$follow = $this->staff_model->get($this->input->post('followers_id'));
		$eco = $this->staff_model->get($this->input->post('handover_recipients'));

		$message = "سلام همکار گرامی ".$staff_request->firstname." ".$staff_request->lastname." درخواست مرخصی شما برای مورخ ".$this->input->post('start_time')." الی ".$this->input->post('end_time')." ثبت شد ، بعد از تایید مرخصی شما از طریق مدیر مربوطه ".$eco->firstname." ".$eco->lastname." از طریق واتس اپ یا پنل خود به شما اطلاع داده خواهد شد";

        $CI = &get_instance();
        $CI->{'sms_msg91'}->send([
                'phone' =>  preg_replace("/^0/", '98', convert2english(str_pad($staff_request->phonenumber, 11, '0', STR_PAD_LEFT))),
                'body'  =>  $message
            ],
            'message'
        );			

        $messageـadmin = "سلام مدیر محترم همکار گرامی ".$staff_request->firstname." ".$staff_request->lastname." درخواست مرخصی دادن موضوع مرخصی ".$this->input->post('subject')." در تاریخ ".$this->input->post('start_time')." الی ".$this->input->post('end_time')." لطفا برای تایید وارد پنل شوید و نسبت به تایید و رد مرخصی اقدام فرمایید";

        $CI = &get_instance();
        $CI->{'sms_msg91'}->send([
                'phone' =>  preg_replace("/^0/", '98', convert2english(str_pad("989902211201", 11, '0', STR_PAD_LEFT))),
                'body'  =>  $messageـadmin
            ],
            'message'
        );        
	...
	public function requisition_manage(){
		...
		$this->load->model('departments_model');
		$data['departments'] = $this->departments_model->get();
		$data['current_date'] = to_view_date_custom(date('Y-m-d H:i:s'));
			...
			if($access_history){
				foreach ($access_history as $key => $value) {
					if($value['type_check'] == '1'){
						$access_history_string .= '<li class="list-group-item"><i class="fa fa-sign-in text-success" aria-hidden="true"></i> '.to_view_date_time_custom($value['date']).'</li>';
					}else{
						$access_history_string .= '<li class="list-group-item"><i class="fa fa-sign-out text-danger" aria-hidden="true"></i> '.to_view_date_time_custom($value['date']).'</li>';
					}
				}
			}
...
public function get_data_additional_timesheets($id){
	...
	if($additional_timesheets){ 
		...
		<td>'. to_view_date_custom($additional_timesheets->additional_day).'</td>
		</tr>
		<tr class="project-overview">
	...
	if(count($list_approve_status) > 0){
		...
		foreach ($list_approve_status as $value) {
			...
			if($value['approve'] == 1){
				...
				$html .= '<br><br>  
				<p class="bold text-center text-success">'. to_view_date_time_custom($value['date']).'</p> 
				';

			}elseif($value['approve'] == 2){ 
				$html .= '<img src="'.site_url(TIMESHEETS_PATH.'approval/rejected.png').'" class="wh-150-80">';
				$html .= '<br><br>  
				<p class="bold text-center text-danger">'. to_view_date_time_custom($value['date']).'</p> 
				';
			}  
			$html .= '</div>'; 
		}
	...
	*/
	...
	public function show_detail_timesheets_mem(){
		...
		$title = get_staff_full_name($member_id). ' - '. to_view_date_custom($d);
		...
		if($access_history){
			foreach ($access_history as $key => $value) {
				if($value['type'] == 'in'){
					$access_history_string .= '<li class="list-group-item"><i class="fa fa-sign-in text-success" aria-hidden="true"></i> '.to_view_date_time_custom($value['time']).'</li>';
				}else{
					$access_history_string .= '<li class="list-group-item"><i class="fa fa-sign-out text-danger" aria-hidden="true"></i> '.to_view_date_time_custom($value['time']).'</li>';
				}
			}
		}
		if($access_history_string != ''){
	...
	public function get_date_leave(){
		...
		echo json_encode([
			'end_date' => to_view_date_custom($end_date)
		]);
		die;    
	}
	/**
	...
	function shift_table(){
		if ($this->input->is_ajax_request()) {
			if($this->input->post()){
				...
				foreach ($rResult as $aRow) {
					$row = [];
					$row[] = to_view_date_custom($aRow['from_date']); 
					$row[] = to_view_date_custom($aRow['to_date']); 
					$department_name = '';
					...
					$row[] = to_view_date_custom($aRow['date_create']); 
	...
	public function requisition_report(){
		if ($this->input->is_ajax_request()) {
			...
			foreach ($rResult as $aRow) {
				...
				$row[] = $aRow['subject'];
				$row[] = to_view_date_custom($aRow['start_time']);  
				$row[] = to_view_date_custom($aRow['end_time']);  
		...
		public function get_data_attendance(){
			if($this->input->post()){
				...
				foreach ($data_check_in_out as $key => $value) {
					...
					$html_list .= '<div class="row"><div class="col-md-12"><div class="alert '.$alert_type.'">'.$type_check.': '.to_view_date_time_custom($value['date']).'</div></div></div>';
				} 
				echo json_encode([
					'html_list' => $html_list,
...
public function route_management(){
	...
	if ($data['tab'] == 'route') {
		...
		foreach ($list_date as $key => $date) {
			array_push($header, to_view_date_custom($date));
		}
	...
	foreach ($staff_route_list as $key => $row) {
		...
		if($data_route){
			...
			$coordinates_list[] = [
				...
				'date_work' => (string)to_view_date_custom($current_date),
...
public function get_check_in_out_history(){
	...
	foreach ($data_check_in_out as $key => $value) {
		...
		$content .= '<div class="col-md-12"><div class="alert '.$alert_type.'">'.$type_check.': '.to_view_date_time_custom($value['date']).'</div></div>';
	} 
	echo json_encode([
	...
	public function history_check_in_out_report(){
		if ($this->input->is_ajax_request()) {
			...
			foreach ($rResult as $aRow) {
				...
				$row[] = to_view_date_time_custom($aRow['date']);
				$type_check = '';
